...
## Log in
The first time the application runs it walks you through creating an admin account. On later launches, enter your username and password. After 5 wrong passwords in a row your account locks for 15 minutes.
...
Double-click a client to open their detail view. Drag and drop one or more statement files onto the big drop zone — PDFs, CSVs, or XLSX files are all accepted. The application:
...
If you drop the same file twice, the application detects it and skips the import.
...
## Export
Click **Export…** and pick Excel, CSV (Google Sheets), or plain text. The Excel workbook has four sheets:

- **Transactions** — every row with date, description, merchant, amount, category, group, certainty, method.
